...
<script type="text/javascript" src="{{ config('usersmanagement.datatablesJsResponsiveCDN') }}"></script>
<script type="text/javascript" src="{{ config('usersmanagement.datatablesJsRespBootCDN') }}"></script>

<script type="text/javascript">
    $(document).ready(function() {
		$('.data-table thead tr').clone(true).addClass("uk-visible@m").appendTo( '.data-table thead' );
		$('.data-table thead tr:eq(1) th').each( function (i) {
			...
			if($(this).hasClass('no-search')){
				...
				var title = $(this).text();
				$(this).html( '<input type="text" placeholder="Filtra '+title+'" style="width:100%" />' );
