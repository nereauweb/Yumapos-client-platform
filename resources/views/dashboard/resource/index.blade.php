            ...
            <div class="card-body">
                ...
                @if( $enableButtons['add'] == 1 )
                    <div class="col-12">
                        ...
                        >
                            {{ __('coreuiforms.resource.add_new') }}
                        </a>
                    </div>
                @endif
                <div class="row">
                            ...
                            <tbody>
                                <?php
                                    foreach($datas as $data){
                                        ...
                                        if($enableButtons['read'] == 1){
                                            echo '<td>';
                                            echo '<a href="' . route("resource.show", ['table' => $form->id, 'resource' => $data['id'] ] ) . '" class="btn btn-primary">' . __('coreuiforms.view') . '</a>';
                                            echo '</td>';
                                        }
                                        if($enableButtons['edit'] == 1){
                                            echo '<td>';
                                            echo '<a href="' . route("resource.edit", ['table' => $form->id, 'resource' => $data['id'] ] ) . '" class="btn btn-primary">' . __('coreuiforms.edit') . '</a>';
                                            echo '</td>';
                                        }
                                        if($enableButtons['delete'] == 1){
                                            echo '<td>';
                                            echo '<form action="' . route("resource.destroy", ['table' => $form->id, 'resource' => $data['id'] ] )  . '" method="POST">';
                                                ?>
                                                    ...
                                                    @method('DELETE')
                                                <?php
                                            echo '<button class="btn btn-danger">' . __('coreuiforms.delete') . '</button>';
                                            echo '</form>';
                                            echo '</td>';
                                        }
                                        echo '</tr>';
